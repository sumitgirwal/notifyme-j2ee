 ...
 <!DOCTYPE html>
    ...
    <title>Organization DashBoard | Edit User</title>
...
%>
 ...
 <div class="form-row">
    ...
    <select class="form-control" name="aname">
        ...
        %>
      <option value="<%=rsa.getString("aname")%>"><%=(rsa.getString("aname")).toUpperCase()%></option>
    <%}}%>
    </select>
  </div> </div>
  ...
  if(!cname.equalsIgnoreCase(rsc.getString("cname"))){
        %>
      <option value="<%=rsc.getString("cname")%>"><%=(rsc.getString("cname")).toUpperCase()%></option>
    <%}}%>
    </select>
  </div>
    </div>
 ...
 <div class="form-row">
  ...
  <textarea class="form-control" id="exampleFormControlTextarea1" rows="5"  name="problem" ><%=rs.getString("problem")%></textarea>        
        ...
        <br>
       <a class="btn btn-outline-info btn-lg" href="showuser.jsp?uid=<%=uid%>" role="button"   >BACK</a> 
    &nbsp;
          <a class="btn btn-outline-info btn-lg" href="deleteuser.jsp?uid=<%=uid%>" role="button"  onclick="return confirm('Are You Sure To Delete Account ?');" >DELETE</a> 
